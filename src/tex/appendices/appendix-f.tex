...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/static/timeline_ranges.png}
    \caption{Timeline ranges across scenarios showing 5th--95th percentile credible intervals. Even the optimistic scenario shows substantial extinction probability within 1000 years.}
    \label{fig:timeline-ranges}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.45\textwidth]{figures/static/montecarlo_time_dist.png}
    \includegraphics[width=0.45\textwidth]{figures/static/montecarlo_cycles_dist.png}
    \caption{Baseline Monte Carlo distributions. Left: time to extinction (years) showing right-skewed distribution with median 432 years. Right: number of corruption-TCS cycles before extinction.}
    \label{fig:montecarlo-distributions}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/static/montecarlo_dashboard.png}
    \caption{Monte Carlo simulation dashboard showing key statistics, parameter sensitivity, and state transition frequencies from 100,000 simulations.}
    \label{fig:montecarlo-dashboard}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/static/scale_degradation_curves.png}
    \caption{Scale-dependent institutional degradation: the four mechanisms that cause cooperation to fail beyond Dunbar scale ($\sim$150). Each mechanism degrades as population increases, with combined effectiveness dropping to $\sim$22\% at $N=1000$ and $\sim$4\% at $N=10000$.}
    \label{fig:scale-degradation}
\end{figure}
