...
# Generated figures - Monte Carlo analysis (require simulation data)
MONTECARLO_FIGURES = \
	$(FIGURES_DIR)/scenario_comparison.png \
	$(FIGURES_DIR)/timeline_ranges.png \
	$(FIGURES_DIR)/montecarlo_time_dist.png \
	$(FIGURES_DIR)/montecarlo_cycles_dist.png \
	$(FIGURES_DIR)/montecarlo_dashboard.png

# All generated figures
GENERATED_FIGURES = $(ABM_FIGURES) $(DIAGRAM_FIGURES) $(MONTECARLO_FIGURES)
...
figures-force: $(BUILD_DIR)/.models-built
	...
	@echo ""
	@echo "--- Go Monte Carlo Simulations ---"
	cd $(MODELS_DIR) && $(MAKE) montecarlo-scenarios
	@echo ""
	@echo "--- Monte Carlo Visualization ---"
	...
	cd $(MODELS_DIR) && docker-compose run --rm montecarlo-viz python -m python.analysis.visualize_montecarlo --csv /app/data/montecarlo_results.csv --json /app/data/montecarlo_stats.json --output /app/figures
	@echo ""
	@echo "✓ All 13 figures regenerated"
...
# Monte Carlo data files - generated by Go simulations
...
# Monte Carlo data depends on Go sources
$(MONTECARLO_DATA): $(GO_SOURCES) $(BUILD_DIR)/.models-built | $(DATA_DIR)
	@echo "Running Go Monte Carlo simulations..."
	cd $(MODELS_DIR) && $(MAKE) montecarlo-scenarios

$(DATA_DIR):
	@mkdir -p $(DATA_DIR)

# Monte Carlo figures - depend on data from Go simulations and Python analysis scripts
$(FIGURES_DIR)/scenario_comparison.png $(FIGURES_DIR)/timeline_ranges.png: $(MONTECARLO_DATA) $(PYTHON_ANALYSIS_SOURCES) $(BUILD_DIR)/.models-built | $(FIGURES_DIR)
	@echo "Generating scenario comparison figures..."
	cd $(MODELS_DIR) && docker-compose run --rm scenario-compare python -m python.analysis.compare_scenarios --data-dir /app/data --output /app/figures

$(FIGURES_DIR)/montecarlo_time_dist.png $(FIGURES_DIR)/montecarlo_cycles_dist.png $(FIGURES_DIR)/montecarlo_dashboard.png: $(MONTECARLO_DATA) $(PYTHON_ANALYSIS_SOURCES) $(BUILD_DIR)/.models-built | $(FIGURES_DIR)
	@echo "Generating Monte Carlo visualization figures..."
	cd $(MODELS_DIR) && docker-compose run --rm montecarlo-viz python -m python.analysis.visualize_montecarlo --csv /app/data/montecarlo_results.csv --json /app/data/montecarlo_stats.json --output /app/figures
