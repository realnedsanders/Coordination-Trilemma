...
\textbf{What we prove here}

This appendix provides mathematical formulations and proofs for core claims:

1. \textbf{The coordination trilemma is logically inescapable}
2. \textbf{Technological Control State leads inevitably to catastrophe}
3. \textbf{The default trajectory terminates with probability $\to$ 1}
4. \textbf{Cooperation fails at scale without transformation}
5. \textbf{Voluntary coordination is the only viable alternative}

\textbf{Epistemological honesty.}
...
All formal notation and definitions are provided in the Glossary (page~\pageref{glossary}). Key notation used throughout:
\begin{itemize}
 \item $C = (A, R, E, M)$ denotes a coordination system
 \item $\theta$ denotes proportion of population (cooperators or value-transformed agents)
 \item $\theta^*$ or $\theta_{\text{crit}}$ denotes critical threshold for stable cooperation
 \item See Glossary for complete definitions and notation reference
\end{itemize}
...
\textbf{Minimal form of assumptions}
...
\textbf{What would falsify these assumptions?}
...
\textbf{Foundational definitions}

The formal definitions of coordination systems, defection, and corruption are provided in the Glossary (page~\pageref{glossary}). We use the standard notation: a coordination system is a tuple $C = (A, R, E, M)$ where $A$ is the set of agents, $R$ is the set of rules, $E$ is the enforcement function, and $M$ is the motivation function.

\textbf{The impossibility theorem}
...
\bigskip
\textbf{Theorem~\ref{thm:trilemma} (Coordination Trilemma):}
\label{thm:trilemma}
...
\textbf{TCS definition and states}
...
This recreates the trilemma at controller level (Theorem~\ref{thm:trilemma}):
\begin{itemize}
...
Regress terminates at some controller subset with no oversight. By Theorem~\ref{thm:trilemma}, corruption occurs with probability:
...
\textbf{Extraction system dynamics.}
...
\bigskip
\textbf{Theorem~\ref{thm:extraction} (Extraction System Instability):}
...
\bigskip
\textbf{Theorem~\ref{thm:terminus} (Default Trajectory Terminus):}
\label{thm:terminus}
...
From $S_{TCS}^H$:
\begin{itemize}
 \item Probability 1 of eventual controller corruption (Theorem~\ref{thm:tcs}, Case 1) $\to$ return to $S_C$
\end{itemize}

From $S_{TCS}^{AI}$:
- Probability 1 of transition to $S_E$ (Theorem~\ref{thm:tcs}, Case 2)  $\to$ \textbf{Absorbing state}
...
\textbf{Why cooperation fails without transformation.}
...
\textbf{Conditions for voluntary coordination stability.}
...
\textbf{Soteriological framework definition.}
...
\bigskip
\textbf{Theorem~\ref{thm:resolution} (Soteriological Resolution):}
...
\textbf{Stakes and decision theory.}
...
Given that:
\begin{enumerate}
 \item The default trajectory inevitably leads to extinction or enslavement (Theorem~\ref{thm:terminus})
 \item Voluntary coordination is the only viable alternative (Theorems 1.1, 2.1)
 \item Voluntary coordination requires true soteriological framework (Theorem~\ref{thm:resolution})
\end{enumerate}
...
\textbf{Types of normative claims.}
...
\textbf{Type 4 (Mathematical) - PROVEN:}
 ...
 \item Cooperation requires $M > c$ (mathematical fact, Theorem~\ref{thm:cooperation})
 \item Default trajectory terminates in catastrophe (proven, Theorem~\ref{thm:terminus})
 \end{itemize}
...
\textbf{Why Type 3 (telic oughtness) is the minimum.}
...
- \textbf{Therefore: VCS is impossible} (Theorem~\ref{thm:resolution} fails - no true framework to discover)
...
\begin{enumerate}
 \item Mathematical coordination facts are objective (proven above)
 \item Human survival desire is empirically universal
 \item VCS is the only path to survival (proven above)
...
\begin{enumerate}
 ...
 \item Scale threshold $|A| > 10^7$
 \item Time horizon $T > 100$ years
\end{enumerate}
...
\emph{Falsification:} Propose a mechanism handling defection at scale that:
 ...
 \item Survives formal analysis in Appendix~\ref{app:a} framework
 \end{itemize}
...
1. \textbf{The trilemma} establishes fundamental constraints on coordination
2. \textbf{TCS instability} eliminates technological control as viable
3. \textbf{Trajectory inevitability} shows default path terminates in catastrophe
4. \textbf{Game theory} shows cooperation requires transformation
5. \textbf{Resolution theorem} proves VCS can work IF conditions are met
6. \textbf{Stakes analysis} shows attempting VCS is rational regardless of success probability
...
\textbf{The metaphysical implication.}
...
\begin{enumerate}
 \item VCS requires a true soteriological framework with $\varphi(S)$ = 1 (Theorem~\ref{thm:resolution})
